...
def test_get_notes_from_notebook():
    ...
    n1.encrypt()
    n1.save_ciphertext()
    assert os.path.exists(os.path.join(nl.get_notesdir(), 'testgetnotesNB', n1.filename ))

    n2 = nl.Notes(title='note2', plaintext='Hello World')
    n2.encrypt()
    n2.save_ciphertext()
    assert os.path.exists(os.path.join(nl.get_notesdir(), 'testgetnotesNB', n2.filename ))
